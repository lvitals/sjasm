...
void RawSource::_Group1(string &line)
{
  ...
  {
    int r = 0;
    if (ifstack.empty() || ifstack.top()._active)
      r = group1tab.exec(d, p, this);
    else if (!(r = group1iftab.exec(d, p, this)))
      r = group2iftab.exec(d, p, this);
    if (r)
    {
      checkjunk(p);
      return;
...
int RawSource::_ParseLabel(string &line)
{
  ...
  if (!mlp.empty())
  {
    ...
    else if (need(line, const_cast<char *>(".@")))
    {
      do
      {
        ...
        if (loc != string::npos)
          mlp = mlp.substr(0, loc);
        else
        {
          mlp.clear(); /*error("No nested macro");*/
          break;
        }
      } while (cneed(line, '@'));
      line = '.' + line;
  ...
  if (needfield(line))
  {
    ...
    checkjunk(s);
    labtab.setvalue(lnr, mapadr, page);
    mapadr += val;
  ...
  else if (needset(line))
  {
    ...
    if (!lnr)
      lnr = labtab.insert(n, true);
    if (!lnr)
    {
      error("dirSET", ERRINTERNAL);
      return 0;
    }
    string s = line;
    int val;
    ...
    labtab.setforward(lnr, labelnotfound != 0);
    checkjunk(s);
    labtab.setvalue(lnr, val, page, true);
...
void RawSource::_ParseRepeat(string &line)
{
  if (sbcneed(line, '['))
  {
    ...
    if (rsr->GetSource().empty())
      error("Nothing to repeat");
    else
      _source.push_back(new Repeat(count, *rsr->GetSource().begin()));
    _source.back()->Process();
  }
  else
    _ExpandMacros(line);
...
void RawSource::_ExpandMacros(string &line)
{
  if (!sbcneed(line, '@'))
  {
    if (mactab.emit(line, this))
      return;
  }
  if (structtab.emit(line, this))
    return;
  if (_AssemblerDirectives(line))
    return;
  _Mnemonics(line);
}
...
int RawSource::_AssemblerDirectives(string &line)
{
  ...
  if (!d.empty())
  {
    if (dirfuntab.exec(d, p, this))
      return 1;
    if (datafuntab.exec(d, p, this))
      return 1;
  }
  return 0;
}
...
void RSStructure::_Parse(string &line)
{
  ...
  if (line[0] != ' ')
  {
    ...
    n = getid(line);
    if (n.empty())
      error1("Illegal label name");
    else
    {
      if (_global || modlabp.empty())
        def.clear();
      else
        def = modlabp + '.';
      def += _s.getname() + '.' + n;
      ...
      if (lnr)
        labtab.setvalue(lnr, 0, 0);
      else
        error("Label already exists", def);
    }
  }
  ...
  if (i)
  {
    ...
    case 4:
      if (!line.empty())
      {
        int d;
        string arg;
        if (!ParseExpression(arg = getargument(line), d))
          error1("Default value expected");
        if (labelnotfound)
    ...
    case 6:
      ...
      {
        al = 1;
        error1("Alignment value expected");
      }
      else
      {
        if (labelnotfound)
        ...
        {
          al = 1;
          error1("Alignment should be positive");
        }
      }
      _newalign = al;
      break;
  ...
  {
    if (_offset % al)
      _offset += al - abs(_offset % al);
    if (lnr)
      labtab.setvalue(lnr, _offset, 0);
