...
public:
  ...
  Data &getdata(int offset, int length)
  {
    Data *datap = new Data(_data.getdatap(offset), length);
    return *datap;
  }
...
{
public:
  Page(int n_pagenr, int n_org, int n_size) : _pagenr(n_pagenr), _org(n_org), _size(n_size), _usedsize(0), _highestad(0)
  {
    _part.clear();
    _opart.clear(); /*_pool.clear();*/
    _relorg = (_org == -1);
  }
  void setsize(int);
...
{
public:
  Output(string n_filename, FILEMODE n_mode = OVERWRITE) : _filename(n_filename), _mode(n_mode), _rn(0), _outp(0), _org(-1)
  {
    _page.clear();
    _rout.clear();
  }
  Rout *getroutp()
  {
    if (_rout.empty())
    {
      _rout.push_back(new Rout());
      _rn = 0;
    }
    return _outp = _rout[_rn];
  }
  Rout *getnewroutp()
  {
    _rout.push_back(new Rout());
    _rn = (int)_rout.size() - 1;
    return _outp = _rout[_rn];
  }
  ...
  int getpageorg(int p)
  {
    if (_page.empty() || (unsigned)p > _page.size())
      return 0;
    return _page[p]->getorg();
  }
  int pageok(int np)
  {
    if (_page.empty() || (unsigned)np >= _page.size())
      return 0;
    return _page[np] != 0;
  }
  void emit(Data &e)
  ...
  void emitblock(int d, int len)
  {
    Data e;
    while (len--)
      e.push((unsigned char)d);
    emit(e);
  }
  void reset();
  void dump(StringList &);
